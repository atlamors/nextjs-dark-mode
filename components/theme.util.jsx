...
export default function SetTheme() {
	...
	useEffect( () => {
		...
		const watchTheme = window.matchMedia('(prefers-color-scheme: dark)')
		const watchWindow = window
		

		// Event listener for system color scheme changing
		watchTheme.addEventListener( 'change', useSetTheme )
		watchWindow.addEventListener( 'unload', useSetTheme )
		...
		// Return and remove eventListener(s) to prevent memory leaks
		return () => {
			watchTheme.removeEventListener( 'change', useSetTheme )
			watchWindow.removeEventListener( 'unload', useSetTheme )
		}
	}, [theme] )
	...
	return (
		...
		{`
		let themeSystem = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'
		let beforeTheme = localStorage.getItem('theme') ?? themeSystem
		document.querySelector(':root').dataset.theme = beforeTheme
		`}
		</Script>
